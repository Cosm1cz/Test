...
import com.cacheFramework.exception.CacheException;
import com.cacheFramework.exception.CacheInvokeException;
import com.cacheFramework.external.AbstractExternalCache;
import com.cacheFramework.nativeCache.AbstractEmbeddedCache;
import com.cacheFramework.util.JetCacheExecutor;
...
public class RefreshCache<K, V> extends LoadingCache<K, V> {
    ...
    //把key和loader存进一个任务列表Map里面，然后进行利用SchedledExecutorPool进行周期性调度
    protected void addOrUpdateRefreshTask(K key, CacheLoader<K,V> loader) {
        ...
        if (refreshMillis > 0) {
            ...
            //RefreshTask里面会去计算上次loader时间，然后判断是否需要重新loader,从而实现了Refresh的功能
